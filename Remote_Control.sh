...
nipe_ask() {
	local decision=1
	while [ $decision == 1 ] # So long as desicion is 1, the while loop will continue.
	do
...
nipe_inst() {
	...
	mkdir /opt/nipe
	git clone https://github.com/htrgouvea/nipe.git /opt/nipe/ && cd /opt/nipe/nipe/ # Clone nipe from github under /opt/ and move to the newly created directory.
	...
	perl /opt/nipe/nipe/nipe.pl install && cd $starting_path
	echo -e "${GS} ${BoG}nipe${NEU} installation was successful!\n"
	echo -e "${BYI} Would you like to update your db?"
	read -p $'\e[1;40;93mINPUT\e[0m Would you line to update your DB? ' update_db
