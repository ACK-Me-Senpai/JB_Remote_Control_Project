...
#
# When we need to transfer files from the remote server to our machine, we can use nc.
# 	source : https://nmap.org/ncat/guide/ncat-file-transfer.html
...
ex_ssh() {
	echo -e "\n${BYI} Gimme the IP address or the domain name of the server you wanna SSH into."
	read -p "    > " middle_man
	echo -e "${BYI} Can you give me the username with which we'll SSH into the server?"
	read -p "    > " username
	echo -e "${BYI} What is his password?"
	read -p "    > " password
	echo -e "\n${YS} Attempting to SSH to ${username}@${middle_man}..."
	echo -e "\n${YS} This is the end of this version. No SSH for you! (right now)"
}
