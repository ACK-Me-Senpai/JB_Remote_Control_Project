...
check_tools () {
	...
	declare -a tool=(nmap whois sshpass geoiplookup) # List of apps required installed via apt-get
	...
	for val in ${tool[@]}; do # Loop for every item in tool list.
		if ! command -v $val &> /dev/null # If the tool does not exist:
		then
	...
	then
		...
		echo -e "${GS} All the tools are installed!" # print a message to varify everything's fine.
...
sshpass_inst() {
	echo -e "${YS} Attempting to install ${BoG}sshpass${NEU} via apt-get. This might take up to a few minutes..."
	if apt-cache search sshpass | grep sshpass &> /dev/null
	then
		echo -e "${YS} ${BoG}sshpass${NEU} exists in apt packages. Proceeding to install..." 
		#apt-get install sshpass -y
		echo -e "${GS} ${BoG}sshpass${NEU} was installed successfully!\n"
	else
		inst_err_apt sshpass
	fi
}

geoiplookup_inst() {
	echo -e "${YS} Attempting to install ${BoG}geoip-bin${NEU} via apt-get. This might take up to a few minutes..."
	if apt-cache search geoip-bin | grep geoip-bin &> /dev/null
	then
		echo -e "${YS} ${BoG}geoip-bin${NEU} exists in apt packages. Proceeding to install..."
		#apt-get install geoip-bin -y &> /dev/null
		echo -e "${GS} ${BoG}geoip-bin${NEU} was installed successfully!\n"
	else
		inst_err_apt geoip-bin
	fi
}

whois_inst() {
	echo -e "${YS} Attempting to install ${BoG}whois${NEU} via apt-get. This might take up to a few minutes..."
	if apt-cache search whois | grep whois &> /dev/null
	then
		echo -e "${YS} ${BoG}whois${NEU} exists in apt packages. Proceeding to install..."
		#apt-get install whois -y &> /dev/null
		echo -e "${GS} ${BoG}whois${NEU} was installed successfully!\n"
	else
		inst_err_apt whois
	fi
}
